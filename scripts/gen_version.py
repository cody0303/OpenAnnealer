...
# --match restricts `git describe` to this project's own "annealer-vX.Y" tags rather
# than the "vX.Y" tags inherited from upstream OpenTrickler (v0.1..v1.20, predating the
# fork) - same reasoning as CMakeLists.txt's own OTA version derivation. Without this,
# git describe picks whichever tag is nearest by commit distance, which silently became
# "annealer-v0.1" once that tag was created - and the regex below, still expecting the
# old plain "vX.Y" format, then failed to match at all, permanently falling back to
# "unknown" for the human-readable firmware version shown in the web UI/REST.
GIT_VERSION_COMMAND = ["git", "describe", "--tags", "--long", "--dirty", "--always", "--match", "annealer-v*"]

GIT_VERSION_PATTERN_REGEX = r'annealer-v(?P<major>\d+)\.(?P<minor>\d+)-(?P<patch>\d+)-g(?P<short_hash>[a-f0-9]+)(?P<dirty>-dirty)?'
